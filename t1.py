...
#     def __init__(self):
#         # Stronger MobileNetV2 variant via width multiplier alpha
#         # Valid alpha: 0.35, 0.5, 0.75, 1.0, 1.3, 1.4 (with include_top=True and imagenet weights) [web:24]
#         self.input_size = 224  # canonical size for include_top=True [web:24]
#         self.model = MobileNetV2(weights="imagenet", alpha=1.4, include_top=True)  # higher capacity backbone [web:24]
#         self.face_cascade = cv2.CascadeClassifier(cv2.data.haarcascades + 'haarcascade_frontalface_default.xml')
#
#     def _resize_rgb(self, img_bgr):
#         img_resized = cv2.resize(img_bgr, (self.input_size, self.input_size))
#         img_rgb = cv2.cvtColor(img_resized, cv2.COLOR_BGR2RGB)
#         return img_rgb
#
#     def _predict_with_tta(self, img_rgb):
#         # Build 4-view TTA: original, hflip, center-crop (slight), center-crop+hflip
#         views = []
#
#         # original
...
#
#         # horizontal flip
#         views.append(cv2.flip(img_rgb, 1))
#
#         # light center-crop -> resize back to 224 to reduce edge artifacts
#         h, w, _ = img_rgb.shape
#         ch0 = int(h * 0.05); cw0 = int(w * 0.05); ch1 = int(h * 0.95); cw1 = int(w * 0.95)
...
#         batch = np.stack(views, axis=0).astype("float32")
#         batch = preprocess_input(batch)  # scales to [-1, 1] as required [web:24]
#
#         # Predict probabilities for all views and average
#         preds = self.model.predict(batch, verbose=0)  # shape (4, 1000)
#         probs = preds.mean(axis=0, keepdims=True)     # shape (1, 1000)
#         return probs
#
#     def check_frame(self, image_data):
#         try:
#             # Decode base64 image
...
#
#             # Prepare input
#             img_rgb = self._resize_rgb(img)
#
#             # AI Object Detection with TTA
#             probs = self._predict_with_tta(img_rgb)
#             decoded = decode_predictions(probs, top=3)[0]  # consistent ImageNet labels [web:24]
#
#             # Head pose detection (basic face detection)
#             gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
#             faces = self.face_cascade.detectMultiScale(gray, 1.1, 4)
#             head_pose_ok = len(faces) == 1  # Exactly one face detected
#
#             # Calculate suspicion score
#             top_pred = decoded[0]
#             confidence = float(top_pred[2])
#
#             suspicious_classes = ['desktop_computer', 'laptop', 'cellular_telephone', 'screen']
#             class_suspicion = 0.3 if any(cls in top_pred[1].lower() for cls in suspicious_classes) else 0
#             pose_suspicion = 0.2 if not head_pose_ok else 0
#             confidence_suspicion = max(0, 0.5 - confidence)
#
#             total_suspicion = min(1.0, class_suspicion + pose_suspicion + confidence_suspicion)
#
#             return {
#                 'suspicion': total_suspicion,
#                 'detected_class': top_pred[1],
#                 'confidence': confidence,
#                 'head_pose_flag': not head_pose_ok,
#                 'audio_flag': False,
#                 'deepfake_flag': False,
#                 'predictions': [(label, desc, float(score)) for label, desc, score in decoded]
#             }
#
...
class AIDetectionEngine:
    def __init__(self, use_tracking=False, imgsz=640, conf=0.25, iou=0.45):
        ...
        # Load YOLOv8n model
        self.model = YOLO("yolov8l.pt")  # auto-downloads weights [web:86]
